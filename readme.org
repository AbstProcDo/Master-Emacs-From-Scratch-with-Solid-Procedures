...
比如从目录下打开文件, 可以M-x find-file, 需要键入十一个字符
Ctrl策略, 可以C-x C-f. 只需要键入四个字符.
"C-x C-f"称为key sequence, 而这种通过key sequnces调用functions的过程, 称之为Command. 而Command=Contrl, 是选择Ctrl键的原因.
